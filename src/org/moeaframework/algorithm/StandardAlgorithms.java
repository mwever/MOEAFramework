...
public class StandardAlgorithms extends AlgorithmProvider {
	...
	public Algorithm getAlgorithm(final String name, final Properties properties, final Problem problem, final Initialization initialization) {
		...
		try {
			if (name.equalsIgnoreCase("MOEAD") || name.equalsIgnoreCase("MOEA/D")) {
			...
			} else if (name.equalsIgnoreCase("CMA-ES") || name.equalsIgnoreCase("CMAES") || name.equalsIgnoreCase("MO-CMA-ES")) {
				...
				return this.newMSOPS(typedProperties, problem);
			} else if (name.equalsIgnoreCase("Random")) {
				return this.newRandomSearch(typedProperties, problem);
			} else if (name.equalsIgnoreCase("DifferentialEvolution") || name.equalsIgnoreCase("DE") || name.equalsIgnoreCase("DE/rand/1/bin")) {
				return this.newDifferentialEvolution(typedProperties, problem);
			} else if (name.equalsIgnoreCase("GeneticAlgorithm") || name.equalsIgnoreCase("GA")) {
				return this.newGeneticAlgorithm(typedProperties, problem, initialization);
			} else if (name.equalsIgnoreCase("EvolutionStrategy") || name.equalsIgnoreCase("ES")) {
				return this.newEvolutionaryStrategy(typedProperties, problem);
			} else if (name.equalsIgnoreCase("RSO")) {
				return this.newRSO(typedProperties, problem);
	 ...
	 *         with the specified type; {@code false} otherwise
	 */
	private boolean checkType(final Class<? extends Variable> type, final Problem problem) {
	 ...
	 */
	private Algorithm neweMOEA(final TypedProperties properties, final Problem problem) {
	 ...
	 */
	private Algorithm newNSGAIII(final TypedProperties properties, final Problem problem) {
		...
		if (problem.getNumberOfConstraints() == 0) {
			selection = new Selection() {
				...
				@Override
				public Solution[] select(final int arity, final Population population) {
			...
			selection = new TournamentSelection(2, new ChainedComparator(new AggregateConstraintComparator(), new DominanceComparator() {
	 ...
	 */
	private Algorithm newMOEAD(final TypedProperties properties, final Problem problem) {
		int populationSize = (int) properties.getDouble("populationSize", 100);
		...
		if ((operator == null) && this.checkType(RealVariable.class, problem)) {
			operator = "de+pm";
		}
		...
		if (properties.contains("neighborhoodSize")) {
			neighborhoodSize = Math.max(2, (int) (properties.getDouble("neighborhoodSize", 0.1) * populationSize));
		}
		...
		MOEAD algorithm = new MOEAD(problem, neighborhoodSize, initialization, variation, properties.getDouble("delta", 0.9), eta, (int) properties.getDouble("updateUtility", -1));
	 ...
	 * @throws FrameworkException if the decision variables are not real valued
	 */
	private Algorithm newGDE3(final TypedProperties properties, final Problem problem) {
		...
		DifferentialEvolutionVariation variation = (DifferentialEvolutionVariation) OperatorFactory.getInstance().getVariation("de", properties, problem);
	 ...
	 */
	private Algorithm neweNSGAII(final TypedProperties properties, final Problem problem) {
		int populationSize = (int) properties.getDouble("populationSize", 100);
		...
		AdaptiveTimeContinuation algorithm = new AdaptiveTimeContinuation(nsgaii, properties.getInt("windowSize", 100),
				Math.max(properties.getInt("windowSize", 100), properties.getInt("maxWindowSize", 100)), 1.0 / properties.getDouble("injectionRate", 0.25),
				properties.getInt("minimumPopulationSize", 100), properties.getInt("maximumPopulationSize", 10000), new UniformSelection(), new UM(1.0));
	 ...
	 */
	private Algorithm newCMAES(final TypedProperties properties, final Problem problem) {
		if (!this.checkType(RealVariable.class, problem)) {
			throw new FrameworkException("unsupported decision variable type");
		}
		...
		if (problem.getNumberOfObjectives() == 1) {
			...
			archive = new EpsilonBoxDominanceArchive(properties.getDoubleArray("epsilon", new double[] { EpsilonHelper.getEpsilon(problem) }));
		}
	 ...
	 */
	private Algorithm newSPEA2(final TypedProperties properties, final Problem problem) {
		int populationSize = (int) properties.getDouble("populationSize", 100);
	 ...
	 */
	private Algorithm newPAES(final TypedProperties properties, final Problem problem) {
	 ...
	 */
	private Algorithm newPESA2(final TypedProperties properties, final Problem problem) {
		int populationSize = (int) properties.getDouble("populationSize", 100);
		int archiveSize = (int) properties.getDouble("archiveSize", 100);
		int bisections = (int) properties.getDouble("bisections", 8);
	 ...
	 */
	private Algorithm newOMOPSO(final TypedProperties properties, final Problem problem) {
		if (!this.checkType(RealVariable.class, problem)) {
			throw new FrameworkException("unsupported decision variable type");
		}
	 ...
	 */
	private Algorithm newSMPSO(final TypedProperties properties, final Problem problem) {
		if (!this.checkType(RealVariable.class, problem)) {
			throw new FrameworkException("unsupported decision variable type");
		}
	 ...
	 */
	private Algorithm newIBEA(final TypedProperties properties, final Problem problem) {
		if (problem.getNumberOfConstraints() > 0) {
	 ...
	 */
	private Algorithm newSMSEMOA(final TypedProperties properties, final Problem problem) {
		int populationSize = (int) properties.getDouble("populationSize", 100);
	 ...
	 */
	private Algorithm newVEGA(final TypedProperties properties, final Problem problem) {
		int populationSize = (int) properties.getDouble("populationSize", 100);
	 ...
	 */
	private Algorithm newDBEA(final TypedProperties properties, final Problem problem) {
		int divisionsOuter = 4;
		int divisionsInner = 0;
	 ...
	 */
	private Algorithm newRVEA(final TypedProperties properties, final Problem problem) {
		int divisionsOuter = 4;
		int divisionsInner = 0;
	 ...
	 */
	private Algorithm newRandomSearch(final TypedProperties properties, final Problem problem) {
		int populationSize = (int) properties.getDouble("populationSize", 100);
	 ...
	 */
	private Algorithm newMSOPS(final TypedProperties properties, final Problem problem) {
		if (!this.checkType(RealVariable.class, problem)) {
			throw new FrameworkException("unsupported decision variable type");
		}
	 ...
	 */
	private Algorithm newRSO(final TypedProperties properties, final Problem problem) {
		...
		return new RepeatedSingleObjective(problem, algorithmName, properties.getProperties(), instances);
	}
	 ...
	 */
	private Algorithm newGeneticAlgorithm(final TypedProperties properties, final Problem problem, final Initialization initialization) {
		int populationSize = (int) properties.getDouble("populationSize", 100);
		...
		return new GeneticAlgorithm(problem, comparator, initializationToBeUsed, selection, variation);
	}
	 ...
	 */
	private Algorithm newEvolutionaryStrategy(final TypedProperties properties, final Problem problem) {
		if (!this.checkType(RealVariable.class, problem)) {
			throw new FrameworkException("unsupported decision variable type");
		}
	 ...
	 */
	private Algorithm newDifferentialEvolution(final TypedProperties properties, final Problem problem) {
		if (!this.checkType(RealVariable.class, problem)) {
			throw new FrameworkException("unsupported decision variable type");
		}
