...
import org.uma.jmetal.algorithm.multiobjective.cdg.CDG;
...
public class JMetalAlgorithms extends AlgorithmProvider {
	...
	public Algorithm getAlgorithm(final String name, final Properties properties, final Problem problem, final Initialization initialization) {
		...
		try {
			...
			} else if (name.equalsIgnoreCase("CDG") || name.equalsIgnoreCase("CDG-JMetal")) {
				algorithm = this.newCDG(typedProperties, adapter);
			...
			} else if (name.equalsIgnoreCase("ESPEA") || name.equalsIgnoreCase("ESPEA-JMetal")) {
				algorithm = this.newESPEA(typedProperties, adapter);
			...
			} else if (name.equalsIgnoreCase("MOEAD") || name.equalsIgnoreCase("MOEAD-JMetal")) {
				algorithm = this.newMOEAD(typedProperties, adapter);
			} else if (name.equalsIgnoreCase("NSGAII") || name.equalsIgnoreCase("NSGAII-JMetal")) {
				algorithm = this.newNSGAII(typedProperties, adapter);
			} else if (name.equalsIgnoreCase("NSGAIII") || name.equalsIgnoreCase("NSGAIII-JMetal")) {
				algorithm = this.newNSGAIII(typedProperties, adapter);
		...
		if (algorithm == null) {
			...
			return new JMetalAlgorithmAdapter(algorithm, (int) typedProperties.getDouble("maxEvaluations", 25000), adapter);
		}
	}
	 ...
	 */
	private ABYSS newAbYSS(final TypedProperties properties, final ProblemAdapter<?> problem) throws JMetalException {
		...
		Archive<DoubleSolution> archive = new CrowdingDistanceArchive<DoubleSolution>((int) properties.getDouble("archiveSize", 100));

		CrossoverOperator<DoubleSolution> crossover = new SBXCrossover(properties.getDouble("sbx.rate", 1.0), properties.getDouble("sbx.distributionIndex", 15.0));

		MutationOperator<DoubleSolution> mutation = new PolynomialMutation(properties.getDouble("pm.rate", 1.0 / problem.getNumberOfVariables()), properties.getDouble("pm.distributionIndex", 20.0));

		ABYSSBuilder builder = new ABYSSBuilder((DoubleProblemAdapter) problem, archive).setMaxEvaluations((int) properties.getDouble("maxEvaluations", 25000))
				.setPopulationSize((int) properties.getDouble("populationSize", 20)).setArchiveSize((int) properties.getDouble("archiveSize", 100))
				.setRefSet1Size((int) properties.getDouble("refSet1Size", 10)).setRefSet2Size((int) properties.getDouble("refSet2Size", 10)).setCrossoverOperator(crossover)
				.setMutationOperator(mutation).setNumberOfSubranges(properties.getInt("numberOfSubRanges", 4));
	...
	private AbstractCDG newCDG(final TypedProperties properties, final ProblemAdapter<?> problem) throws JMetalException {
		if (!(problem instanceof DoubleProblemAdapter)) {
			throw new JMetalException("CDG only supports problems with real decision variables");
		}

		DifferentialEvolutionCrossover crossover = new DifferentialEvolutionCrossover(properties.getDouble("de.crossoverRate", 0.1), properties.getDouble("de.stepSize", 0.5),
				properties.getString("de.variant", "rand/1/bin"));

		CDGBuilder builder = new CDGBuilder((DoubleProblemAdapter) problem).setCrossover(crossover).setMaxEvaluations((int) properties.getDouble("maxEvaluations", 25000))
				.setPopulationSize((int) properties.getDouble("populationSize", 100)).setResultPopulationSize((int) properties.getDouble("archiveSize", 100))
	 ...
	 * @throws JMetalException if an error occurred when constructing the algorithm
	 */
	private CellDE45 newCellDE(final TypedProperties properties, final ProblemAdapter<?> problem) throws JMetalException {
		if (!(problem instanceof DoubleProblemAdapter)) {
		...
		return new CellDE45((DoubleProblemAdapter) problem, (int) properties.getDouble("maxEvaluations", 25000), (int) properties.getDouble("populationSize", 100),
				new CrowdingDistanceArchive<DoubleSolution>((int) properties.getDouble("archiveSize", 100)), new C9<DoubleSolution>((int) Math.sqrt(100), (int) Math.sqrt(100)), selection, crossover,
				(int) properties.getDouble("feedBack", 20), evaluator);
	}
	...
	private ESPEA newESPEA(final TypedProperties properties, final ProblemAdapter<?> problem) throws JMetalException {
	 ...
	 * @throws JMetalException if an error occurred when constructing the algorithm
	 */
	private GDE3 newGDE3(final TypedProperties properties, final ProblemAdapter<?> problem) throws JMetalException {
		if (!(problem instanceof DoubleProblemAdapter)) {
		...
		GDE3Builder builder = new GDE3Builder((DoubleProblemAdapter) problem).setCrossover(crossover).setSelection(new DifferentialEvolutionSelection())
				.setMaxEvaluations((int) properties.getDouble("maxEvaluations", 25000)).setPopulationSize((int) properties.getDouble("populationSize", 100));
	 ...
	 */
	@SuppressWarnings({ "rawtypes", "unchecked" })
	private IBEA newIBEA(final TypedProperties properties, final ProblemAdapter<?> problem) throws JMetalException {
		CrossoverOperator<?> crossover = JMetalFactory.getInstance().createCrossoverOperator(problem, properties);
		MutationOperator<?> mutation = JMetalFactory.getInstance().createMutationOperator(problem, properties);
		SelectionOperator selection = new BinaryTournamentSelection();

		return new IBEA(problem, (int) properties.getDouble("populationSize", 100), (int) properties.getDouble("archiveSize", 100), (int) properties.getDouble("maxEvaluations", 25000), selection,
				crossover, mutation);
	}
	 ...
	 */
	@SuppressWarnings({ "rawtypes", "unchecked" })
	private MOCell newMOCell(final TypedProperties properties, final ProblemAdapter<?> problem) throws JMetalException {
		...
		MOCellBuilder builder = new MOCellBuilder(problem, crossover, mutation).setSelectionOperator(selection).setMaxEvaluations(maxEvaluations).setPopulationSize(populationSize)
	 ...
	 */
	@SuppressWarnings({ "rawtypes", "unchecked" })
	private MOCHC newMOCHC(final TypedProperties properties, final ProblemAdapter<?> problem) throws JMetalException {
		...
		MOCHCBuilder builder = new MOCHCBuilder((BinaryProblemAdapter) problem).setInitialConvergenceCount(properties.getDouble("initialConvergenceCount", 0.25))
				.setConvergenceValue((int) properties.getDouble("convergenceValue", 3)).setPreservedPopulation(properties.getDouble("preservedPopulation", 0.05))
				.setMaxEvaluations((int) properties.getDouble("maxEvaluations", 25000)).setPopulationSize(populationSize).setCrossover(crossover).setNewGenerationSelection(newGenerationSelection)
				.setCataclysmicMutation(mutation).setParentSelection(parentSelection);
	 ...
	 */
	@SuppressWarnings({ "unchecked", "rawtypes" })
	private AbstractMOEAD newMOEAD(final TypedProperties properties, final ProblemAdapter<?> problem) throws JMetalException {
		if (!(problem instanceof DoubleProblemAdapter)) {
		...
		MOEADBuilder builder = new MOEADBuilder((DoubleProblemAdapter) problem, MOEADBuilder.Variant.MOEAD).setCrossover(crossover).setMutation(mutation)
				.setMaxEvaluations((int) properties.getDouble("maxEvaluations", 25000)).setPopulationSize((int) properties.getDouble("populationSize", 100))
				.setResultPopulationSize((int) properties.getDouble("archiveSize", 100)).setNeighborhoodSelectionProbability(properties.getDouble("delta", 0.9))
				.setMaximumNumberOfReplacedSolutions(properties.getInt("eta", 2)).setNeighborSize((int) properties.getDouble("neighborhoodSize", 20)).setFunctionType(functionType)
	 ...
	 */
	@SuppressWarnings({ "unchecked", "rawtypes" })
	private NSGAII newNSGAII(final TypedProperties properties, final ProblemAdapter<?> problem) throws JMetalException {
		CrossoverOperator<?> crossover = JMetalFactory.getInstance().createCrossoverOperator(problem, properties);
		MutationOperator<?> mutation = JMetalFactory.getInstance().createMutationOperator(problem, properties);
		SelectionOperator selection = new BinaryTournamentSelection();
		...
		NSGAIIBuilder builder = new NSGAIIBuilder(problem, crossover, mutation, populationSize).setSelectionOperator(selection).setMaxEvaluations(maxEvaluations);
	 ...
	 */
	@SuppressWarnings({ "rawtypes", "unchecked" })
	private NSGAIII newNSGAIII(final TypedProperties properties, final ProblemAdapter<?> problem) throws JMetalException {
		CrossoverOperator<?> crossover = JMetalFactory.getInstance().createCrossoverOperator(problem, properties);
		MutationOperator<?> mutation = JMetalFactory.getInstance().createMutationOperator(problem, properties);
		SelectionOperator selection = new BinaryTournamentSelection();
		...
		NSGAIIIBuilder builder = new NSGAIIIBuilder(problem).setCrossoverOperator(crossover).setMutationOperator(mutation).setSelectionOperator(selection).setPopulationSize(populationSize)
	 ...
	 */
	@SuppressWarnings({ "rawtypes", "unchecked" })
	private OMOPSO newOMOPSO(final TypedProperties properties, final ProblemAdapter<?> problem) throws JMetalException {
		if (!(problem instanceof DoubleProblemAdapter)) {
		...
		UniformMutation uniformMutation = new UniformMutation(properties.getDouble("mutationProbability", mutationProbability), properties.getDouble("perturbationIndex", 0.5));

		NonUniformMutation nonUniformMutation = new NonUniformMutation(properties.getDouble("mutationProbability", mutationProbability), properties.getDouble("perturbationIndex", 0.5), maxIterations);

		SolutionListEvaluator evaluator = new SequentialSolutionListEvaluator();

		OMOPSOBuilder builder = new OMOPSOBuilder((DoubleProblemAdapter) problem, evaluator).setMaxIterations(maxIterations).setSwarmSize(populationSize).setArchiveSize(archiveSize)
				.setUniformMutation(uniformMutation).setNonUniformMutation(nonUniformMutation);
	 ...
	 */
	@SuppressWarnings({ "unchecked", "rawtypes" })
	private PAES newPAES(final TypedProperties properties, final ProblemAdapter<?> problem) throws JMetalException {
		MutationOperator<?> mutation = JMetalFactory.getInstance().createMutationOperator(problem, properties);

		PAESBuilder builder = new PAESBuilder(problem).setMutationOperator(mutation).setMaxEvaluations((int) properties.getDouble("maxEvaluations", 25000))
				.setArchiveSize((int) properties.getDouble("archiveSize", 100)).setBiSections((int) properties.getDouble("bisections", 8));
	 ...
	 */
	@SuppressWarnings({ "rawtypes", "unchecked" })
	private PESA2 newPESA2(final TypedProperties properties, final ProblemAdapter<?> problem) throws JMetalException {
		CrossoverOperator<?> crossover = JMetalFactory.getInstance().createCrossoverOperator(problem, properties);
		MutationOperator<?> mutation = JMetalFactory.getInstance().createMutationOperator(problem, properties);

		PESA2Builder builder = new PESA2Builder(problem, crossover, mutation).setMaxEvaluations((int) properties.getDouble("maxEvaluations", 25000))
				.setPopulationSize((int) properties.getDouble("populationSize", 10)).setArchiveSize((int) properties.getDouble("archiveSize", 100))
	 ...
	 */
	@SuppressWarnings({ "rawtypes", "unchecked" })
	private SMPSO newSMPSO(final TypedProperties properties, final ProblemAdapter<?> problem) throws JMetalException {
		if (!(problem instanceof DoubleProblemAdapter)) {
			throw new JMetalException("SMPSO only supports problems with real decision variables");
		}

		MutationOperator<DoubleSolution> mutation = (MutationOperator<DoubleSolution>) JMetalFactory.getInstance().createMutationOperator(problem, properties);
		...
		SMPSOBuilder builder = new SMPSOBuilder((DoubleProblemAdapter) problem, archive).setMutation(mutation).setMaxIterations(maxIterations).setSwarmSize(populationSize);
	 ...
	 */
	@SuppressWarnings({ "unchecked", "rawtypes" })
	private SPEA2 newSPEA2(final TypedProperties properties, final ProblemAdapter<?> problem) throws JMetalException {
		CrossoverOperator<?> crossover = JMetalFactory.getInstance().createCrossoverOperator(problem, properties);
		MutationOperator<?> mutation = JMetalFactory.getInstance().createMutationOperator(problem, properties);
		SelectionOperator selection = new BinaryTournamentSelection();
		...
		SPEA2Builder builder = new SPEA2Builder(problem, crossover, mutation).setSelectionOperator(selection).setMaxIterations(maxEvaluations / populationSize).setPopulationSize(populationSize)
	 ...
	 */
	@SuppressWarnings({ "unchecked", "rawtypes" })
	private SMSEMOA newSMSEMOA(final TypedProperties properties, final ProblemAdapter<?> problem) throws JMetalException {
		CrossoverOperator<?> crossover = JMetalFactory.getInstance().createCrossoverOperator(problem, properties);
		MutationOperator<?> mutation = JMetalFactory.getInstance().createMutationOperator(problem, properties);
		...
		SMSEMOABuilder builder = new SMSEMOABuilder(problem, crossover, mutation).setSelectionOperator(selection).setMaxEvaluations((int) properties.getDouble("maxEvaluations", 25000))
				.setPopulationSize((int) properties.getDouble("populationSize", 100)).setHypervolumeImplementation(hypervolume);
